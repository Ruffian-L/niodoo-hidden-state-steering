...
EXPECTED="${2:-3}"
# Durable, human-readable evidence — never /tmp (a black box that hides the run
# and gets wiped on reboot). STANDARDS.md: evidence is plain-text and kept.
RUN_ID="${NIODOO_RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
OUT="${NIODOO_OUT_DIR:-$REPO/evidence/reproduce/run_$RUN_ID}"
mkdir -p "$OUT"
