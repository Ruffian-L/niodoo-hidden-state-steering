...
fn compute_attention(gaussian_idx: u32) -> array<vec4<f32>, FEATURE_DIM_VEC4> {
    ...
    for (var i = 0u; i < PSC_BANK_SIZE; i = i + 1u) {
        ...
        for (var q = 0u; q < QUERY_DIM_VEC4; q = q + 1u) {
            dot_prod = dot_prod + dot(g_mem.query_vector[q], psc_bank[i].features[q]);
        }
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // TODO: Project position and calculate alpha (placeholder for now)
    output_features[idx].projected_pos = vec2<f32>(0.0, 0.0);
    output_features[idx].final_alpha = 1.0;
}
